...
object Finagle {
  ...
  def fromHttp4sRequest[F[_]: ConcurrentEffect](
      req: Request[F],
      streaming: Boolean): F[FRequest] = {
  ...
  def toFinagleResponse[F[_]: ConcurrentEffect](
      response: Response[F],
  ...
  def mkService[F[_]: ConcurrentEffect](
      routes: HttpRoutes[F],
  ...
  def mkClient[F[_]: ConcurrentEffect](
  ...
  def unsafeReadBodyStream[F[_]](body: EntityBody[F])(
      implicit F: ConcurrentEffect[F]): Reader[Buf] = {
